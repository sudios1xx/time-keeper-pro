...
const PlayerEventsPage: React.FC = () => {
  ...
  const eventosFiltrados = useMemo(
    () =>
      eventos.filter(
        (evento) => filterType === "todos" || evento.tipo === filterType,
      ),
    [eventos, filterType],
  );
  ...
  const estatisticas = useMemo(() => {
    ...
    const eventosConfirmados = jogadorData
      ? eventos.filter(
          (e) =>
            e.presencas.find((p) => p.jogadorId === jogadorData.id)?.confirmou,
        ).length
      : 0;
  ...
  const stats = [
    ...
    {
      title: "Confirmados",
      value: estatisticas.eventosConfirmados,
      icon: CheckCircle,
      color: "success",
    },
    {
      title: "Próximos",
      value: estatisticas.proximosEventos,
      icon: WarningCircle,
      color: "info",
    },
    {
  ...
  const confirmarPresenca = (eventoId: string, confirmar: boolean) => {
    logger.log(
      `${confirmar ? "Confirmando" : "Cancelando"} presença no evento ${eventoId}`,
    );
  };
  ...
  const getEventIcon = (tipo: string) => {
    switch (tipo) {
      case "social":
        return <Lightning className="w-6 h-6" />;
      case "treino":
            ...
            {tiposEventos.map((tipo) => {
              ...
              return (
                <Button
                  ...
                  className={`${
                    filterType === tipo.value ? "shadow-glow" : ""
      ...
      <div className="grid gap-2 sm:gap-4">
        {eventosFiltrados.map((evento) => {
          const minhaPresenca = jogadorData
            ? evento.presencas.find((p) => p.jogadorId === jogadorData.id)
            : undefined;
          ...
          const percentualConfirmacoes =
            (confirmacoes / totalJogadores) * 100;
                  ...
                  <div className="flex flex-col sm:flex-row items-start justify-between gap-3 sm:gap-0">
                    <div className="flex items-start space-x-3 sm:space-x-4">
                      <div className="relative">
                        <div
                          className={`p-2 sm:p-3 rounded-xl ${getEventColor(evento.tipo)} shadow-card`}
                        >
                          {getEventIcon(evento.tipo)}
                        </div>
                        {isProximo && (
                          <div className="absolute -top-1 -right-1 w-2 sm:w-3 h-2 sm:h-3 bg-success rounded-full animate-pulse-glow" />
                        )}
                      </div>
                  ...
                  <div className="bg-accent/30 rounded-lg p-3 sm:p-4">
                    <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-2 sm:gap-0 mb-3">
                      ...
                      <span className="text-xs sm:text-sm font-medium">
                        {confirmacoes}/{totalJogadores} confirmados (
                        {Math.round(percentualConfirmacoes)}%)
                      </span>
                    </div>
                    ...
                    <div className="grid grid-cols-3 gap-2 sm:gap-4 text-center text-xs sm:text-sm">
                      ...
                      <div className="hover:bg-accent/30 rounded-md p-1.5 transition-colors cursor-pointer">
                        <Badge
                          variant={
                            percentualConfirmacoes >= 80
                              ? "default"
                              : percentualConfirmacoes >= 60
                                ? "secondary"
                                : "destructive"
                          }
                          className="text-xs"
