...
const JogosJogador: React.FC = () => {
  ...
  useEffect(() => {
    let active = true;
    (async () => {
      const [players, games] = await Promise.all([data.getPlayers(), data.getGames()]);
      if (!active) return;
  ...
  const jogosFiltrados = useMemo(() => jogos.filter(jogo => 
  ...
  const estatisticas = useMemo(() => ({
    ...
    minhaPresenca: jogadorData
      ? jogos.filter(j => j.presencas.find(p => p.jogadorId === jogadorData.id)?.confirmou).length
      : 0
  }), [jogos, jogadorData]);

  const stats = [
    { title: 'Total', value: estatisticas.totalJogos, icon: Calendar, color: 'blue' },
    { title: 'Próximos', value: estatisticas.jogosAgendados, icon: Clock, color: 'info' },
    { title: 'Confirmei', value: estatisticas.minhaPresenca, icon: CheckCircle, color: 'success' },
    { title: 'Presença', value: `${jogadorData?.percentualPresenca ?? 0}%`, icon: TrendUp, color: 'warning' },
  ];

  const confirmarPresenca = (jogoId: string, confirmar: boolean) => {
    // Em uma aplicação real, isso seria uma chamada para API
    logger.log(`${confirmar ? 'Confirmando' : 'Cancelando'} presença no jogo ${jogoId}`);
    ...
    <div className="space-y-3 sm:space-y-6 animate-fade-in px-1">
      ...
      <div className="grid gap-3 sm:gap-4">
        {jogosFiltrados.map((jogo) => {
          const minhaPresenca = jogadorData ? jogo.presencas.find(p => p.jogadorId === jogadorData.id) : undefined;
